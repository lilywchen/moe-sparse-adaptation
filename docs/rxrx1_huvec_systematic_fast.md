...
classification difficulty, then test a matched dense/MoE pair only after ViT-Micro and ViT-Tiny
source-IID plateau studies establish a well-optimized training setup.

For a non-specialist visual explanation of experiments, wells, sites, channels, custom splits,
Cell-DINO difficulty, preliminary evidence, and the plateau-stopped training study, open
...
6. Run paired ViT-Micro and ViT-Tiny recipe studies until source-IID performance plateaus.
