...
controlled characterization. We first require a pretrained transformer to demonstrate competent
dense full fine-tuning on each dataset; a powerful natural-image model is not assumed to transfer
to a scientific modality merely because it is a foundation model. For each qualified substrate, we
change exactly one feed-forward block and hold the total parameter budget fixed to the nearest
realizable dense width. We then vary four interpretable decisions as a full factorial: placement,
routing unit, router geometry, and training pressure.
