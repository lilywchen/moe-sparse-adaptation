...
from scripts import sweep_rxrx1_temperature03_aux as sweep
from scripts.sweep_rxrx1_router_aux import sharded_rows


def test_temperature03_addendum_is_bounded_unique_and_disjoint():
    rows = sweep.cells()
    assert len(rows) == 4
    assert len({run_id for _, _, run_id in rows}) == 4
    assert {tag for tag, _, _ in rows} == {
        f"route_E{n}_temp03_{aux}"
...
def test_temperature03_addendum_changes_only_predeclared_fields():
    for tag, overrides, run_id in sweep.cells():
        ...
        assert "temperature03_aux60" in run_id


def test_temperature03_addendum_shards_are_one_cell_and_exhaustive():
    rows = sweep.cells()
