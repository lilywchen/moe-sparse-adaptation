...
from scripts import sweep_rxrx1_temperature03_canonical_aux as extreme_canonical
from scripts import sweep_rxrx1_temperature03_moderate_bank_aux as sweep
from scripts import sweep_rxrx1_temperature03_aux as extreme_route
from scripts.sweep_rxrx1_router_aux import sharded_rows


def test_moderate_bank_temperature03_family_is_bounded_unique_and_disjoint():
    rows = sweep.cells()
    assert len(rows) == 12
    assert len({run_id for _, _, run_id in rows}) == 12
    extreme_ids = {
        run_id
        for family in (extreme_route.cells(), extreme_canonical.cells())
        for _, _, run_id in family
    }
    assert {run_id for _, _, run_id in rows}.isdisjoint(extreme_ids)
    assert {tag for tag, _, _ in rows} == {
        f"{pressure}_E{n}_temp03_{aux}"
...
def test_moderate_bank_temperature03_family_changes_only_predeclared_fields():
    for tag, overrides, run_id in sweep.cells():
        ...
        assert "temperature03_moderate_bank_aux60" in run_id


def test_moderate_bank_temperature03_family_shards_one_cell_exhaustively():
    rows = sweep.cells()
    shards = [sharded_rows(rows, index, 12) for index in range(12)]
