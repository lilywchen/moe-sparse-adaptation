...
from scripts import sweep_rxrx1_temperature_expert_count_e2_confirm as sweep


def test_e2_confirmation_is_locked_and_uses_shared_e8_anchors():
    rows = sweep.cells()
    assert len(rows) == 2
    ...
    for tag, overrides, run_id, comparator in rows:
        ...
        seed = cfg["seed"]
        assert tag == f"route_E2_s{seed}"
        assert cfg["model"]["n_experts"] == 2
        ...
        assert f"route_E2k1_ep60_s{seed}_temperature_expert_count_confirm60" in run_id


def test_e2_launch_adapter_preserves_registry_identity():
