...
from scripts.certify_rxrx1_huvec_recipe import default_recipes, show_status
...
def _all_status(result_root):
    for pair_index in range(3):
        for item in pair_plan(pair_index):
            print(f"===== {item['run_name']} / {item['model']} =====", flush=True)
            show_status(result_root, item["run_name"], item["model"])
