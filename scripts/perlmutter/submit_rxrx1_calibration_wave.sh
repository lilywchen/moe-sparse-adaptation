...
submit() {
  ...
  sbatch --parsable -J "$name" -A m1266_g -C gpu -q preempt \
    ...
    --wrap="module load pytorch/2.11.0; CODE=$CODE MANIFEST=$FULL/manifests/all_sites.parquet RAW_ROOT=$FULL/rxrx1 RESULT_ROOT=$RESULT RUN_NAME=$name MODEL=$model NPROC=4 PER_GPU_BATCH=$batch WORKERS=16 EXTRA_ARGS='$extra' bash $CODE/scripts/multicluster/run_rxrx1_calibration.sh"
}
