#!/usr/bin/env python
"""Two-seed expert-count and placement neighbors of shared E3/top-1 late2."""
import argparse
import json
import os
import subprocess
...
from aggregate_shared_neighbors import render_report
...
CAMPAIGN = "shared_neighbors30_20260809"
DEFAULT_ROOT = Path(
...
WANDB_GROUP = "rxrx1-cell-dino-shared-neighbors30-20260809"
HF_PREFIX = "rxrx1/cell_dino_cp5/shared_neighbors30_20260809"
SEEDS = (1, 2)
...
def wave_rows(config=CONFIG):
    specs = (
        ("shared_E2k1_late2", 2, (10, 11)),
        ("shared_E4k1_late2", 4, (10, 11)),
        ("shared_E3k1_block10", 3, (10,)),
        ("shared_E3k1_block11", 3, (11,)),
    )
    rows = []
    for seed in SEEDS:
        for label, n_experts, blocks in specs:
            intervention = [
                "model.variant=shared_moe", f"model.n_experts={n_experts}",
                "model.top_k=1", _blocks(blocks),
            ]
    ...
    if len(run_ids) != len(set(run_ids)):
        raise ValueError("shared-neighbor arms collide on run identity")
    return rows
...
def write_manifest(out, rows):
    ...
    payload = {
        ...
        "questions": [
            "Does constant-active-compute expert-bank scaling improve over E3?",
            "Is block 10 or block 11 responsible for the two-block gain?",
        ],
        "runs": [
            {
                ...
                "ffn_block_indices": cfg["model"]["ffn_block_indices"],
            }
...
def tracking_environment(require_tracking=True):
    ...
    env["WANDB_JOB_TYPE"] = "rxrx1_shared_neighbors30"
    env["WANDB_TAGS"] = "rxrx1,cell-dino,shared-residual,expert-scaling,placement,stage3"
