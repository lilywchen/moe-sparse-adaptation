#!/usr/bin/env python
"""Two-seed routing controls around the replicated shared E3/top-1 late2 winner."""
import argparse
import json
import os
import subprocess
...
from aggregate_shared_routing import render_report
...
CAMPAIGN = "shared_routing30_20260809"
DEFAULT_ROOT = Path(
...
WANDB_GROUP = "rxrx1-cell-dino-shared-routing30-20260809"
HF_PREFIX = "rxrx1/cell_dino_cp5/shared_routing30_20260809"
SEEDS = (1, 2)


def _common(seed, label):
    return [
        f"seed={seed}", "stage=3", "model.variant=shared_moe",
        "model.n_experts=3", "model.top_k=1", "model.ffn_block_indices=[10,11]",
        "model.routing_estimator=selected_st", "model.geometry=cosine",
        ...
        "model.router_frozen=false", "model.routing_unit=token",
...
def wave_rows(config=CONFIG):
    specs = (
        ("shared_E3k1_image", ["model.routing_unit=image"]),
        ("shared_E3k1_balance1e3", ["losses.balance_w=1.0e-3"]),
        ("shared_E3k1_balance0", ["losses.balance_w=0.0"]),
        ("shared_E3k1_router_frozen", ["model.router_frozen=true"]),
    )
    rows = []
    for seed in SEEDS:
    ...
    if len(run_ids) != len(set(run_ids)):
        raise ValueError("shared-routing arms collide on run identity")
    return rows
...
def write_manifest(out, rows):
    ...
    payload = {
        ...
        "questions": [
            "Does routing once per field outperform token routing?",
            "Does weaker or zero load balancing improve conditional specialization?",
            "Does a learned router beat an otherwise identical fixed random partition?",
        ],
        "runs": [
            {
                "label": display, "arm": arm, "seed": seed, "run_id": run_id,
                "overrides": overrides, "routing_unit": cfg["model"]["routing_unit"],
                "router_frozen": bool(cfg["model"].get("router_frozen", False)),
                "balance_w": float(cfg["losses"]["balance_w"]),
                "zloss_w": float(cfg["losses"]["zloss_w"]),
            }
...
def tracking_environment(require_tracking=True):
    ...
    env["WANDB_JOB_TYPE"] = "rxrx1_shared_routing30"
    env["WANDB_TAGS"] = "rxrx1,cell-dino,shared-residual,routing-controls,stage3"
