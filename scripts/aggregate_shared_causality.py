#!/usr/bin/env python
"""One-command status and paired summary for the shared-MoE causality wave."""
import argparse
import json
import math
...
CONTRASTS = (
    ...
    ("shared_E3k1_late2", "shared_E3k1_balance0"),
    ("shared_E3k1_late2", "shared_E3k1_router_frozen"),
)
...
def render_report(result_root):
    ...
    lines.append("Protocol: " + ("; ".join(problems) if problems else
                                  "completed rows pass declared seed/split/stage checks"))
    lines += [
    ...
    for row in rows:
        if row["result"] and row["acc_val"] is not None:
            grouped[row["arm"]].append(row["acc_val"])
    lines += ["", "Across-seed terminal OOD-validation summary:",
              "| Arm | n | Mean | SEM |", "|---|---:|---:|---:|"]
    for arm in ARMS:
