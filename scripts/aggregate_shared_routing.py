#!/usr/bin/env python
"""One-command status and paired summary for the shared-residual routing wave."""
import argparse
import json
import math
...
CONTRASTS = (
    ("shared_E3k1_image", ANCHOR_ARM),
    ("shared_E3k1_balance1e3", ANCHOR_ARM),
    ("shared_E3k1_balance0", ANCHOR_ARM),
    ("shared_E3k1_router_frozen", ANCHOR_ARM),
    ("shared_E3k1_balance0", "shared_E3k1_balance1e3"),
)
...
def _row(root, spec, order):
    ...
    return {
        ...
        **{metric: _metric(result, milestone, metric) for metric in METRICS},
        ...
        "router_delta": (result or {}).get("router_parameter_deltas"),
    }
...
def load(root):
    ...
    return root, manifest, [_row(root, spec, i) for i, spec in enumerate(manifest.get("runs", []))]


def _anchors(manifest):
    root = Path(manifest.get("anchor_root", "")).expanduser()
    anchor_manifest = _json(root / "wave_manifest.json") or {"runs": []}
    return [_row(root, spec, i) for i, spec in enumerate(anchor_manifest.get("runs", []))
            if spec.get("arm") == ANCHOR_ARM
            and int(spec.get("seed", -1)) in manifest.get("seeds", [])]
...
def validate(manifest, rows, anchors):
    ...
    for row in rows:
        ...
        if bool(result.get("router_frozen", False)) != bool(row.get("router_frozen", False)):
            problems.append(f"{row['label']}: router-frozen mismatch")
    return problems
