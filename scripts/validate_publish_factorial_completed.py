#!/usr/bin/env python
"""Strictly validate and publish completed factorial60 runs lacking manifests."""
...
from scripts.sweep_rxrx1_cell_dino import CONFIG, cells
...
REPORT = Path("/home/idies/workspace/hb_factorial_publish0800.json")
VALIDATE_ONLY = os.environ.get("FACTORIAL_VALIDATE_ONLY") == "1"
if VALIDATE_ONLY:
    REPORT = Path("/home/idies/workspace/hb_factorial_validate_all0800.json")
...
def expected_from_tag(tag):
    if tag == "original":
        return {"variant": "original", "params": 22_402_163}
    bits = tag.split("_")
    if bits[0] == "dense":
        return {
            "variant": "dense_wide",
...
for result_path in sorted(ROOT.glob("*.json")):
    ...
    if manifest_path.exists() and not VALIDATE_ONLY:
        continue
    assert rid in registry
    expected = expected_from_tag(registry[rid])
    assert result["dataset"] == "rxrx1" and result["seed"] == 0
    for key, value in expected.items():
        if key != "params":
            assert result[key] == value, (rid, key, result[key], value)
    ...
    assert result["total_params"] == expected["params"]
    # Output-pressure arms own a small adversary head.  Keep the scientific model-size
    # comparison on total_params, while checking the optimizer accounting explicitly.
    assert result["training_total_params"] == (
        expected["params"] + result["adversary_params"]
    )
    assert result["git_sha"] == "b8ece25" and result["git_dirty"] is False
    assert result["tracking"]["group"] == "rxrx1-cell-dino-factorial60-20260801"
    ...
    assert checkpoint_path.is_file() and checkpoint_path.stat().st_size > 200_000_000
    if VALIDATE_ONLY:
        reports.append({
...
print("STRICT_BATCH_VALIDATE_PASS" if VALIDATE_ONLY else "STRICT_BATCH_PUBLISH_PASS", len(reports))
