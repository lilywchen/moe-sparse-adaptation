#!/usr/bin/env python
"""Locked fresh-seed confirmation of route-balanced E32 versus shared E8 anchors.

Route E32 preserved its predeclared worst-experiment advantage from epochs 30
to 60 and reached epoch 60 with positive mean OOD, ID, and worst-experiment
differences versus the exact route-E8 anchor.  This bounded registry freezes
E32 at seeds 1 and 2 and shares the already launched route-E8 confirmation
anchors.  No setting is tuned: route pressure, temperature 0.03, zero balance
loss, z-loss 0.001, optimizer, data order, horizon, and checkpoint policy all
remain fixed.  The estimand is active-compute matched; OOD test is sealed.
"""
import sys
    ...
    for seed in SEEDS:
        overrides = [
            *base,
            ...
            f"run_tag=temperature_expert_count_confirm60_route_E32_s{seed}_20260802",
        ]
        cfg = apply_overrides(load_config(config), overrides)
        rows.append((f"route_E32_s{seed}", overrides, run_id_from(cfg), comparators[seed]))
    return rows
